...
#初始化一个loggin
loggin=get_logger('log1')

#读取Excel数据
cases=Handler.Excle.read_excle("find_community")
@ddt.ddt
class MyTestCase(unittest.TestCase):

    @ddt.data(*cases)
    def test_find_community(self,case_info):
        ...
        try:
            ...
            Handler.Excle.write_excle(Form="find_community", row=case_info["case_id"] + 1, column=9, data="pass")
        except AssertionError as e:
            loggin.error("第{}条测试用例不通过:{}".format(case_info["case_id"], e))
            Handler.Excle.write_excle(Form="find_community", row=case_info["case_id"] + 1, column=9, data="fail")
            raise e
